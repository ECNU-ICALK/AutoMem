...
from automem.evaluation.io import safe_json_loads

from .agents import ToolCallingAgent
from .memory import ActionStep, PlanningStep, SummaryStep, TaskStep
from .search_tools import CrawlPageTool, GitHubIssueTool, GitHubSearchTool, WebSearchTool
...
class MMSearchAgent(BaseAgent):
    def __init__(self, model, summary_interval, prompts_type, max_steps, memory_provider=None, **kwargs):
        ...
        from .mm_tools import AudioInspectorTool, TextInspectorTool, VisualInspectorTool

        allowed_roots = kwargs.get("allowed_roots")
        web_tool = WebSearchTool()
        crawl_tool = CrawlPageTool(model=model)
        visual_tool = VisualInspectorTool(model, 100000, allowed_roots=allowed_roots)
        text_tool = TextInspectorTool(model, 100000, allowed_roots=allowed_roots)
        audio_tool = AudioInspectorTool(model, 100000, allowed_roots=allowed_roots)
