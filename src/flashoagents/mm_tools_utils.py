...
def validated_zip_members(zip_ref: zipfile.ZipFile) -> List[zipfile.ZipInfo]:
    """Validate archive paths and expansion limits before reading members."""
    ...
    for info in members:
        ...
        if file_type == stat.S_IFLNK:
            raise ValueError(f"ZIP symlink members are not allowed: {name!r}")
        if file_type not in (0, stat.S_IFREG, stat.S_IFDIR):
            raise ValueError(f"Unsupported ZIP member type: {name!r}")
        if info.flag_bits & 0x1:
            raise ValueError(f"Encrypted ZIP members are not supported: {name!r}")
        if info.file_size > MAX_ZIP_MEMBER_BYTES:
            raise ValueError(f"ZIP member is too large: {name!r}")
        ...
        if (
            ...
            raise ValueError(f"ZIP compression ratio is unsafe: {name!r}")
    return members


def safe_extract_zip(local_path: str, destination: str) -> List[str]:
    """Extract a validated ZIP into a new caller-owned directory."""
    ...
    if requested_destination.exists() or requested_destination.is_symlink():
        raise ValueError("ZIP destination must be a new, non-symlink directory")
    lexical_parent = requested_destination.parent
    try:
        destination_parent = lexical_parent.resolve(strict=True)
    except (FileNotFoundError, OSError) as exc:
        raise ValueError("ZIP destination parent must already exist") from exc
    if not destination_parent.is_dir():
        raise ValueError("ZIP destination parent is not a directory")
    if destination_parent != lexical_parent:
        raise ValueError("ZIP destination parent path must not contain symlinks")
    destination_path = destination_parent / requested_destination.name
    extracted: List[str] = []
    total_written = 0
    with zipfile.ZipFile(local_path, "r") as zip_ref:
        members = validated_zip_members(zip_ref)
        ...
        for info in members:
            ...
            try:
                ...
                raise ValueError(f"ZIP member escapes destination: {info.filename!r}") from exc
            if info.is_dir():
                target.mkdir(parents=True, exist_ok=True)
                continue
            target.parent.mkdir(parents=True, exist_ok=True)
            written = 0
            with zip_ref.open(info, "r") as source, open(target, "xb") as output:
                while chunk := source.read(1024 * 1024):
                    written += len(chunk)
                    total_written += len(chunk)
                    if written > MAX_ZIP_MEMBER_BYTES:
                        raise ValueError(f"ZIP member exceeded size limit: {info.filename!r}")
                    if total_written > MAX_ZIP_TOTAL_BYTES:
                        raise ValueError("ZIP expanded size exceeded the configured limit")
                    output.write(chunk)
...
class ZipConverter(DocumentConverter):
    ...
    def __init__(self, extract_dir: Optional[str] = None):
        # Kept for API compatibility. Conversion only reports validated member
        # names and no longer writes archive contents to a shared directory.
    ...
    def convert(self, local_path: str, **kwargs: Any) -> Optional[DocumentConverterResult]:
        ...
        with zipfile.ZipFile(local_path, "r") as zip_ref:
            members = [
                info.filename
                for info in validated_zip_members(zip_ref)
                if not info.is_dir()
            ]

        md_content = "Archive contains the following files:\n" + "\n".join(
            f"* {name}" for name in sorted(members)
        )
...
class MarkdownConverter:
    ...
    def convert(self, source: Union[str, requests.Response], **kwargs: Any) -> DocumentConverterResult:
        if isinstance(source, str):
            if source.startswith(("http://", "https://", "file://")):
                raise ValueError(
                    "Remote and file URLs are disabled for document inspection; "
                    "use the dedicated web tools for network resources"
                )
            else:
    ...
    def convert_url(self, url: str, **kwargs: Any) -> DocumentConverterResult:
        raise ValueError(
            "Remote URL conversion is disabled; use the dedicated web tools"
        )
