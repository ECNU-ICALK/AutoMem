...
MAX_INSPECTOR_FILE_BYTES = 100 * 1024 * 1024


def _normalize_allowed_roots(allowed_roots: Optional[Sequence[str]]) -> tuple[Path, ...]:
    roots = []
    for raw_root in allowed_roots or []:
        root = Path(raw_root).expanduser().resolve(strict=True)
        if not root.is_dir():
            raise ValueError(f"Inspector allowed root is not a directory: {root}")
        roots.append(root)
    return tuple(roots)


def _require_allowed_local_file(
    file_path: str,
    allowed_roots: Sequence[Path],
    *,
    max_bytes: int = MAX_INSPECTOR_FILE_BYTES,
) -> str:
    """Resolve one regular file without allowing URL or filesystem escape."""

    if not isinstance(file_path, str) or not file_path.strip() or "\x00" in file_path:
        raise ValueError("file_path must be a non-empty local path")
    lowered = file_path.strip().lower()
    if lowered.startswith(("http://", "https://", "file://")) or "://" in lowered:
        raise ValueError("Inspector tools accept local files only")
    if not allowed_roots:
        raise PermissionError("File inspection is disabled because no allowed root was configured")

    requested = Path(file_path).expanduser()
    candidates = (
        [requested]
        if requested.is_absolute()
        else [root / requested for root in allowed_roots]
    )
    for candidate in candidates:
        ...
        for root in allowed_roots:
            ...
            if resolved.stat().st_size > max_bytes:
                raise ValueError(f"Inspector file exceeds {max_bytes} bytes: {resolved.name}")
            return str(resolved)
    raise PermissionError(f"File is outside configured inspector roots: {file_path!r}")
...
This tool supports the following image formats: [".jpg", ".jpeg", ".png", ".gif", ".bmp"]. For other file types, use the appropriate inspection tool."""
    ...
    def forward(self, file_path: str, question: Optional[str] = None) -> str:
        file_path = _require_allowed_local_file(file_path, self.allowed_roots)
        ...
        try:
            ...
            description = self.model(messages, max_tokens=2000, temperature=0.0).content
...
This tool handles the following file extensions: [".txt", ".csv", ".md", ".log", ".html", ".pdb", ".xlsx", ".xls", ".pdf", ".docx", ".ppt", ".pptx"], and all other types of text files. IT DOES NOT HANDLE IMAGES."""
    ...
    def forward_initial_exam_mode(self, file_path, question):
        ...
        except self._conversion_errors:
        ...
        # Defensive: some upstream parsers (e.g. for jsonld / pdb / nested
        # structured docs) return text_content as a list of strings.
        # Flatten to str before any concatenation / length check below.
        if isinstance(result.text_content, list):
            result.text_content = "\n\n".join(
                str(p) for p in result.text_content if p is not None
            )
...
This tool supports the following audio formats: [".mp3", ".m4a", ".wav"]. For other file types, use the appropriate inspection tool."""
    ...
    def transcribe_audio(self, file_path: str) -> str:
        ...
        if not self.api_key or not self.base_url:
            raise RuntimeError(
                "Audio transcription requires both MTU_API_KEY and MTU_BASE_URL"
            )
    ...
    def forward(self, file_path: str, question: Optional[str] = None) -> str:
        ...
        messages = [
            ...
            },
            {
                ...
                "content": [{
                    "type": "text",
                    "text": "Answer the following question based on the audio content using the format:\n"
                            f"1. Brief answer\n2. Detailed analysis\n3. Relevant context\nQuestion: {question}"
                }]
            }
        ]
...
def get_zip_description(
    ...
    *,
    ...
    from .mm_tools_utils import safe_extract_zip
